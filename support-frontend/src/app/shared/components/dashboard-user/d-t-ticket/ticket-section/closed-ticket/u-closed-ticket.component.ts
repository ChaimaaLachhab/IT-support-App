...
@Component({
  ...
  templateUrl: './u-closed-ticket.component.html',
  styleUrls: ['./u-closed-ticket.component.css']
})
export class UClosedTicketComponent implements OnInit {
  ...
  loadTickets(): void {
    this.supportTicketService.getTicketsByUser().subscribe({
      next: tickets => this.tickets = tickets.filter(ticket => ticket.status === TicketStatus.CLOSED),
