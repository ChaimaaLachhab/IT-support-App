...
@Component({
  ...
  templateUrl: './u-open-ticket.component.html',
  styleUrls: ['./u-open-ticket.component.css']
})
export class UOpenTicketComponent implements OnInit {
  ...
  loadTickets(): void {
    this.supportTicketService.getTicketsByUser().subscribe({
      next: tickets => this.tickets = tickets.filter(ticket => ticket.status === TicketStatus.OPEN),
