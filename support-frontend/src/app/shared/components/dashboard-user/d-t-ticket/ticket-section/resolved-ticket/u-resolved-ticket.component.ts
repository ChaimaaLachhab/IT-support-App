...
@Component({
  selector: 'app-u-resolved-ticket',
  standalone: true,
  imports: [
    CommonModule,
    RouterModule,
  ...
  templateUrl: './u-resolved-ticket.component.html',
  styleUrls: ['./u-resolved-ticket.component.css']
})
export class UResolvedTicketComponent implements OnInit {
  ...
  loadTickets(): void {
    this.supportTicketService.getTicketsByUser().subscribe({
      next: tickets => this.tickets = tickets.filter(ticket => ticket.status === TicketStatus.RESOLVED),
